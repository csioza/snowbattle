...
//        float startZ = MapStartPosition.y;
    ...
    public bool QuickFindPath(Vector3 startPos, Vector3 endPos, bool isSearchAround)
    {
        //Find start and end nodes, if we cant return null and stop!
        if (!isSearchAround)
        {
            startNode = FindClosestNode_Q(startPos);
            if (null == startNode)
            {
                return false;
            }
            endNode = FindClosestNode_Q(endPos);
            if (null == endNode)
            {
                return false;
            }
        ...
        returnPath = FindPath(startPos, endPos);
        ...
        {
            Vector3 tmpPos = returnPath[returnPath.Count - 1];
            float fDist = Vector3.Distance(endPos, tmpPos);
            if (fDist > 0.5f)
            {
                return false;
            }
...
//     {
//         int x = Mathf.Abs(Mathf.FloorToInt((pos.x - MapStartPosition.x) / Tilesize));
//         int z = Mathf.Abs(Mathf.FloorToInt((pos.z - MapStartPosition.y) / Tilesize));
...
//         return n.walkable;
//     }
    private Node FindClosestNode_Q(Vector3 pos)
    {
        int x = Mathf.Abs(Mathf.FloorToInt((pos.x - MapStartPosition.x) / Tilesize));
        int z = Mathf.Abs(Mathf.FloorToInt((pos.z - MapStartPosition.y) / Tilesize));
        if (x < 0 || z < 0 || x > Map.GetLength(0) || z > Map.GetLength(1))
...
//         int z = (MapStartPosition.y < 0F) ? Mathf.FloorToInt(((pos.z + Mathf.Abs(MapStartPosition.y)) / Tilesize)) : Mathf.FloorToInt((pos.z - MapStartPosition.y) / Tilesize);
        ...
        {
            print("not FindClosestNode try look around its neightbours");
...
//         int z = (MapStartPosition.y < 0F) ? Mathf.FloorToInt(((pos.z + Mathf.Abs(MapStartPosition.y)) / Tilesize)) : Mathf.FloorToInt((pos.z - MapStartPosition.y) / Tilesize);
    ...
    void DrawMapLines()
    {
        DrawMapInEditor = GameSettings.Singleton.m_isOpenFathfinderDebug;
        if (DrawMapInEditor == true && Map != null)
        {
            for (int i = 0; i < Map.GetLength(1); i++)
            {
