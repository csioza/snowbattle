...
public class WeaponTrail : MonoBehaviour {
    ...
    */
	...
	public Vector3 rotateTrail = Vector3.zero;

    //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    // 序列帧
    public Vector2 Tiles = new Vector2(1, 1);
    public float playSpeed = 1; // 播放速度(秒)
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    #endregion
    //
    #region Temporary
    ...
    //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    // 序列帧
    float runTime = 0; // 前一次切换时间
    int curFrame = 0;   // 当前使用的纹理坐标
    List<TrailUVTile> uvAry = new List<TrailUVTile>();
    TrailUVTile curUVTile = new TrailUVTile();
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    #endregion
