...
Shader "Toon/Basic_Sphere" {
	Properties {
		_Color ("Main Color", Color) = (.5,.5,.5,1)
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_ToonShade ("ToonShader Sphere(RGB)", 2D) = "white" { }
	...
	SubShader {
		...
		Pass {
			...
			float2 GenRoleSphereMapUV(float4 localNormal)
			{
				float2 sphereMapUV;
				sphereMapUV.xy = mul (UNITY_MATRIX_MV, localNormal).xy;
				sphereMapUV.x = sphereMapUV.x*0.5 + 0.5+_Time.z;
				sphereMapUV.y = sphereMapUV.y*-0.5 + 0.5+_Time.z;
				return sphereMapUV;
			}
			v2f vert (appdata v)
			{
				...
				o.cubenormal = GenRoleSphereMapUV(float4(v.normal,0));
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float4 cube = tex2D(_ToonShade, i.cubenormal);
				//return cube;
				return float4(i.fresnel + col.rgb*2+cube.rgb*cube.a*col.a, col.a);
			}
			ENDCG			
		}
	} 
